...
GO
   CREATE VIEW [dbo].[articles_Adresses] AS
SELECT
   a.AR_Ref,
   a.DE_No,
   a.DP_No,
   ...
   dp.DE_Intitule
FROM
   dbo.F_ARTSTOCKEMPL AS a
   INNER JOIN dbo.F_DEPOTEMPL AS d ON a.DE_No = d.DE_No
   AND a.DP_No = d.DP_No
   INNER JOIN dbo.F_DEPOT AS dp ON a.DE_No = dp.DE_No
GO
...
',
   ...
   @level0name = N'dbo',
   @level1type = N'VIEW',
   @level1name = N'articles_Adresses'
GO
...
SET
   QUOTED_IDENTIFIER ON
GO
   CREATE VIEW [dbo].[fiche_details_Fiche] AS
SELECT
   dbo.fiche.id_Fiche,
   dbo.fiche.date_controle,
   dbo.details_Fiche.AR_Ref,
   dbo.F_ARTICLE.AR_Design,
   dbo.F_COMPTET.CT_Intitule,
   CASE
      WHEN [details_Fiche].etat = 0 THEN 'Fiche en attente envoie'
      WHEN [details_Fiche].etat = 1 THEN 'Fiche non referencier'
      WHEN [details_Fiche].etat = 2 THEN 'Fiche non controler'
      WHEN [details_Fiche].etat = 3 THEN 'Attente validation Resp. Stock'
      WHEN [details_Fiche].etat = - 2 THEN 'Mise en Quarantaine '
      WHEN [details_Fiche].etat = - 3 THEN 'REBUT'
      WHEN [details_Fiche].etat = 4 THEN 'Attente mise en place'
   END AS position,
   dbo.details_Fiche.quantite,
   dbo.details_Fiche.FO_ref,
   dbo.details_Fiche.P_ref,
   dbo.details_Fiche.dosage,
   dbo.details_Fiche.fabricant,
   dbo.details_Fiche.T_Stockage_ref,
   dbo.details_Fiche.volume,
   dbo.details_Fiche.poids,
   dbo.details_Fiche.CT_Num,
   dbo.formes.FO_designation,
   dbo.presentations.P_Intitule,
   dbo.details_Fiche.date_fab,
   dbo.details_Fiche.date_peremp,
   dbo.Type_Stockage.Type_Stockage,
   dbo.details_Fiche.etat,
   dbo.details_Fiche.num_Lot,
   dbo.details_Fiche.dt_Fiche_ref,
   CASE
      WHEN DATEDIFF(month, GETDATE(), dbo.details_Fiche.date_peremp) - (
         datediff(year, GETDATE(), dbo.details_Fiche.date_peremp) * 12
      ) < 0 THEN (
         (
            (
               DATEDIFF(year, GETDATE(), dbo.details_Fiche.date_peremp)
            ) * 12
         ) - ABS(
            (
               DATEDIFF(month, GETDATE(), dbo.details_Fiche.date_peremp) - (
                  datediff(year, GETDATE(), dbo.details_Fiche.date_peremp) * 12
               )
            )
         )
      ) / 12
      ELSE DATEDIFF(year, GETDATE(), dbo.details_Fiche.date_peremp)
   END AS ANS,
   CASE
      WHEN DATEDIFF(month, GETDATE(), dbo.details_Fiche.date_peremp) - (
         datediff(year, GETDATE(), dbo.details_Fiche.date_peremp) * 12
      ...
      ELSE DATEDIFF(month, GETDATE(), dbo.details_Fiche.date_peremp) - (
         datediff(
            year,
            GETDATE(),
            dbo.details_Fiche.date_peremp
   ...
   END AS MOIS,
   dbo.details_Fiche.P_quantite,
   dbo.details_Fiche.Observation
FROM
   dbo.fiche
   INNER JOIN dbo.details_Fiche ON dbo.fiche.id_Fiche = dbo.details_Fiche.id_Fiche
   INNER JOIN dbo.F_ARTICLE ON dbo.details_Fiche.AR_Ref = dbo.F_ARTICLE.AR_Ref
   INNER JOIN dbo.presentations ON dbo.details_Fiche.P_ref = dbo.presentations.P_ref
   INNER JOIN dbo.F_COMPTET ON dbo.details_Fiche.CT_Num = dbo.F_COMPTET.CT_Num
   INNER JOIN dbo.Type_Stockage ON dbo.details_Fiche.T_Stockage_ref = dbo.Type_Stockage.T_Stockage_ref
   INNER JOIN dbo.formes ON dbo.details_Fiche.FO_ref = dbo.formes.FO_ref
GROUP BY
   dbo.fiche.id_Fiche,
   dbo.fiche.date_controle,
   dbo.details_Fiche.AR_Ref,
...
Begin DesignProperties = 
   ...
   @level0name = N'dbo',
   @level1type = N'VIEW',
   @level1name = N'fiche_details_Fiche'
GO
   EXEC sys.sp_addextendedproperty @name = N'MS_DiagramPane2',
   @value = N 'isplayFlags = 280
            TopColumn = 0
         End
      End
   End
...
SET
   QUOTED_IDENTIFIER ON
GO
   CREATE VIEW [dbo].[dt_fiche_scores] AS
SELECT
   dbo.details_fiche_score.dt_fiche_ref,
   dbo.controle_condition.cond_controle_ref,
   dbo.controle_condition.id_libelle,
   dbo.controle_condition.normes,
   dbo.Type_Condition.Libelle,
   dbo.Type_Condition.Notation,
   dbo.details_fiche_score.score,
   dbo.details_fiche_score.observation,
   dbo.details_fiche_score.Condition_ref
FROM
   dbo.details_fiche_score
   INNER JOIN dbo.controle_condition ON dbo.details_fiche_score.Condition_ref = dbo.controle_condition.cond_controle_ref
   INNER JOIN dbo.Type_Condition ON dbo.controle_condition.id_libelle = dbo.Type_Condition.id
GROUP BY
   dbo.details_fiche_score.dt_fiche_ref,
   dbo.controle_condition.cond_controle_ref,
   dbo.controle_condition.id_libelle,
...
SET
   QUOTED_IDENTIFIER ON
GO
   CREATE VIEW [dbo].[details_FCPCC] AS
SELECT
   dbo.fiche_details_Fiche.id_Fiche,
   dbo.fiche_details_Fiche.date_controle,
   dbo.fiche_details_Fiche.AR_Ref,
   dbo.fiche_details_Fiche.AR_Design,
   dbo.fiche_details_Fiche.CT_Intitule,
   dbo.fiche_details_Fiche.quantite,
   dbo.fiche_details_Fiche.FO_ref,
   dbo.fiche_details_Fiche.P_ref,
   dbo.fiche_details_Fiche.dosage,
   dbo.fiche_details_Fiche.fabricant,
   dbo.fiche_details_Fiche.T_Stockage_ref,
   dbo.fiche_details_Fiche.volume,
   dbo.fiche_details_Fiche.poids,
   dbo.fiche_details_Fiche.CT_Num,
   dbo.fiche_details_Fiche.FO_designation,
   dbo.fiche_details_Fiche.P_Intitule,
   dbo.fiche_details_Fiche.date_fab,
   dbo.fiche_details_Fiche.date_peremp,
   dbo.fiche_details_Fiche.Type_Stockage,
   dbo.fiche_details_Fiche.etat,
   dbo.fiche_details_Fiche.num_Lot,
   dbo.fiche_details_Fiche.dt_Fiche_ref,
   dbo.fiche_details_Fiche.ANS,
   dbo.fiche_details_Fiche.MOIS,
   dbo.dt_fiche_scores.normes,
   dbo.dt_fiche_scores.Libelle,
   dbo.dt_fiche_scores.score,
   dbo.dt_fiche_scores.Notation,
   dbo.dt_fiche_scores.observation,
   dbo.dt_fiche_scores.id_libelle,
   dbo.fiche_reference.ref_marche,
   dbo.fiche_reference.date_livraison,
   dbo.F_COMPTET.CT_Intitule AS fournisseur,
   dbo.fiche_details_Fiche.position,
   dbo.fiche_details_Fiche.Observation AS ObsDecision
FROM
   dbo.fiche_details_Fiche
   INNER JOIN dbo.dt_fiche_scores ON dbo.fiche_details_Fiche.dt_Fiche_ref = dbo.dt_fiche_scores.dt_fiche_ref
   INNER JOIN dbo.fiche_reference ON dbo.fiche_details_Fiche.id_Fiche = dbo.fiche_reference.id_Fiche
   INNER JOIN dbo.F_COMPTET ON dbo.fiche_reference.fournisseur_ref = dbo.F_COMPTET.CT_Num
GO
   EXEC sys.sp_addextendedproperty @name = N'MS_DiagramPane1',
   @value = N '[0E232FF0-B466-11cf-A24F-00AA00A3EFFF, 1.00]
Begin DesignProperties = 
...
SET
   QUOTED_IDENTIFIER ON
GO
   CREATE VIEW [dbo].[dt_fiche_stock] AS
SELECT
   dbo.fiche_stock.id_fiche_stock,
   dbo.fiche_details_Fiche.AR_Ref,
   dbo.fiche_details_Fiche.AR_Design,
   dbo.fiche_details_Fiche.quantite,
   dbo.fiche_details_Fiche.P_Intitule,
   ...
   dbo.fiche_details_Fiche.num_Lot,
   dbo.fiche_details_Fiche.dt_Fiche_ref,
   dbo.stock_Empl.num_Rack,
   dbo.stock_Empl.quantite AS qte_sur_rack,
   dbo.stock_Empl.observation,
   dbo.stock_Empl.Date,
   dbo.fiche_stock.DE_No,
   dbo.F_DEPOT.DE_Intitule,
   dbo.stock_Empl.id_stock_Empl
FROM
   dbo.fiche_details_Fiche
   INNER JOIN dbo.fiche_stock ON dbo.fiche_details_Fiche.dt_Fiche_ref = dbo.fiche_stock.dt_Fiche_ref
   INNER JOIN dbo.stock_Empl ON dbo.fiche_stock.id_fiche_stock = dbo.stock_Empl.id_fiche_stock
   INNER JOIN dbo.F_DEPOT ON dbo.fiche_stock.DE_No = dbo.F_DEPOT.DE_No
GO
   EXEC sys.sp_addextendedproperty @name = N'MS_DiagramPane1',
   @value = N '[0E232FF0-B466-11cf-A24F-00AA00A3EFFF, 1.00]
Begin DesignProperties = 
...
SET
   QUOTED_IDENTIFIER ON
GO
   CREATE VIEW [dbo].[mvt_stock] AS
SELECT
   dfs.id_stock_empl,
   dt.id_fiche_stock,
   se.num_Rack,
   dfs.date AS date_mvt,
   dfs.num_Doc,
   dfs.CT_Num,
   cpt.CT_Intitule,
   dfs.entree,
   ...
   dt.AR_Ref,
   dt.num_Lot,
   dbo.details_Fiche.id_Fiche
FROM
   dbo.dt_fiche_stock AS dt
   INNER JOIN dbo.details_Fiche_Stock AS dfs ON dfs.id_stock_empl = dt.id_stock_Empl
   INNER JOIN dbo.fiche_stock AS f ON f.id_fiche_stock = dt.id_fiche_stock
   INNER JOIN dbo.details_Fiche ON f.dt_Fiche_ref = dbo.details_Fiche.dt_Fiche_ref
   INNER JOIN dbo.fiche ON dbo.details_Fiche.id_Fiche = dbo.fiche.id_Fiche
   LEFT OUTER JOIN dbo.F_COMPTET AS cpt ON cpt.CT_Num = dfs.CT_Num
   LEFT OUTER JOIN dbo.stock_Empl AS se ON se.id_stock_Empl = dt.id_stock_Empl
GROUP BY
   dfs.id_stock_empl,
   dt.id_fiche_stock,
   dfs.entree,
